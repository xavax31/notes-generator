...
var jxCore = {
	...
	init: function (params) {
		...
		params.mode = this._getFirstDefined(
			params.mode,
			window["overrideJXConfig"].mode,
			window["jxCore"].urlParams.mode,
			"prod"
		);
		params.debug =
			this._getFirstDefined(
				params.debug,
				window["overrideJXConfig"].debug,
				window["jxCore"].urlParams.debug,
				false
			).toString() == "true";
		...
		params.jxsrc =
			this._getFirstDefined(
				params.jxsrc,
				window["overrideJXConfig"].jxsrc,
				window["jxCore"].urlParams.jxsrc,
				true
			).toString() == "true";
		...
		if (params.mode) {
			...
			requirejs.config({
				...
				//except, for jx path
				paths:
					mode && mode.classesPathes
						? mode.classesPathes
						: {
								jx:
									window["overrideJXConfig"].jxPath ||
									mode.jxPath,
								comps: "src/comps",
								screens: "src/screens",
			...
			requirejs.config({
				...
				paths: {
					jx:
						window["overrideJXConfig"].jxPath ||
						"/dev/mp-framework/current/engine/build/jx",
					comps: "src/comps",
					screens: "src/screens",
					JXEngine: "src/JXEngine",
				},
		...
		if (!params.jxsrc) {
			var jxminPath =
				params.mode == "dev"
					? "/dev/mp-framework/current/lib/jx/jxengine.min.js"
					: "lib/jx/jxengine.min";
	...
	splashScreen: function () {
		...
		this.splashScreenSrc =
			window["overrideJXConfig"].splashScreen ||
			"assets/images/SPLASHSCREEN.png";
		this.startButtonSrc = "assets/images/START_BUTTON.png";

		$("body").append(
			'<div id="splashscreen"  style="display:block; width: 100%; height: 100%; position: absolute; top: 0px; left: 0px; -webkit-background-size: contain; -moz-background-size: contain; -o-background-size: contain; background-size: contain;"></div>'
		);

		$("#splashscreen").append(
			'<div id="startBTN" style="left: 0px; top: 0px; display: none; cursor: default; z-index: 301;"><img id="image" src="' +
				this.startButtonSrc +
				'" style="height: 80%; display: inline;"></div>'
		);
