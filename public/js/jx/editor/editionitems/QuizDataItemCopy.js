    ...
    var QuizDataItem = function (_DataGroup) {
        ...
        _createClass(QuizDataItem, [{
            ...
            value: function clearAll(_ref) {
                ...
                if (confirm) {
                    this.jx.editor.confirm({ content: "Supprimer toutes les questions ?", onclose: function onclose(evt) {
                            if (evt.action == "ok") {
                                for (var i = 0; i < _this2.questionsArr.length; i++) {
                                    _this2.questionsArr[i].view.remove();
                                }
                                _this2.questionsArr.length = 0;
                                _this2.questionsContainer.data.children.length = 0;
                                _this2.questionsContainer.onchanged.dispatch({ target: _this2.questionsContainer });
                            }
                        } });
                } else {
                    for (var i = 0; i < this.questionsArr.length; i++) {
                        this.questionsArr[i].view.remove();
                    }
                    this.questionsArr.length = 0;
                    this.questionsContainer.data.children.length = 0;
            ...
            value: function addQuestion(infos) {
                ...
                this.questionsContainer.data.children.push(data);
                var question = this.questionsContainer.cc({ type: "editionitems.DataGroup", data: data });
                question.onchanged.add(function () {
                    _this3.questionsContainer.onchanged.dispatch({ target: _this3.questionsContainer });
                });
            ...
            value: function _changeQuiz(_ref3) {
                ...
                if (recreateQuiz) {
                    ...
                    this.jx.editor.confirm({ title: "Création du Quiz", content: "Attention ! \nCeci va supprimer les données existantes.\n Voulez-vous continuer ?", onclose: function onclose(evt) {
                            if (evt.action == "ok") {
                                _this9.clearAll({ confirm: false });

                                for (var i = 0; i < _this9._infos.questionsNum; i++) {
                                    _this9.addQuestion(_this9._infos);
                                }
                                _this9.questionsContainer.onchanged.dispatch({ target: _this9.questionsContainer });
                            }
                            ;
                            if (name != _this9.data.title) {
                                _this9.setTitle(name);
                                _this9.questionsContainer.onchanged.dispatch({ target: _this9.questionsContainer });

                                var editionPage = _this9.controller.mainController;
                                var gabaritView = editionPage.gabaritView;
                                editionPage.refreshEditionView();
                                editionPage.showTab(_this9.id);
                            }
                            console.log("configure2", _this9.questionsArr);
                        } });
                }
                ;
            }
            ...
            value: function deleteQuiz() {
                ...
                this.jx.editor.confirm({ title: "Suppression du Quiz", content: "Attention ! \nCeci va supprimer définitivement ce Quiz.\n Voulez-vous continuer ?", onclose: function onclose(evt) {
                        if (evt.action == "ok") {
                            var editionPage = _this10.controller.mainController;
                            editionPage.deleteTab(_this10.id);
                        }
                        ;
                    } });
            }
...
//# sourceMappingURL=QuizDataItemCopy.js.map